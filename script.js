$(document).ready(function () {
  ...
  colorList.map((c, i) => {
    colorCont.append(() => {
      return `<div class="color" style="background:${c}" data-colVal="${c}" alt="${c}"></div>`;
    });
  });
  //default color
  ...
  addIcon.click((e) => {
    ...
    if (e && !confirmOpen) {
      ...
      if (colorPallet.length >= 1) {
        ...
        $(".cp-cont").append(
          `<div class="cp-color" data-colorid="${colorObj._id}" style="background:${colorObj.color}">
              <p class="cp-i-text" style="${getColorText(colorObj.color)}">${colorObj.color}</p>
              <div class="cp-c-options">
                <i class="far fa-copy click-icon cpy-btn" data-copyid="${colorObj.color}"></i>
                <i class="far fa-trash-alt click-icon" data-deleteid="${colorObj._id}"></i>
              </div> 
           </div>`
        );
      }
      checkUpdate();
      // set confirmation
      $(".confirmation").text(`Color ${colorObj.color} Has Been added to your pallet!`);
      $(".confirmation").addClass("c-open");
      confirmOpen = false;
      setTimeout(() => {
        $(".confirmation").removeClass("c-open");
  ...
  // add way to just copy all hex codes
  $('#copy-all').click((e) => {
    var copyText = `Your HEX codes are: ${colorPallet.map(d => {
      return `${d.color}`
    })}`
